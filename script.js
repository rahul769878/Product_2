...
function displayMain(d) {
  
  let display = d.map(function(item,index){
    return `<div class="products-container" >
    <div class="card" style="width: 18rem;">
  <img src="${item.image}" class="card-img-top" alt="...">
  <div class="card-body">
    <h5 class="card-title">${item.title}</h5>
    <p class="card-text" id="${index}" onclick="readMore(${index})">${item.description.slice(0,50).concat("...readMore")}</p>
    <a href="#" class="btn btn-primary" onclick="addToCart(${index})">$${item.price}</a>
  </div>
</div>
    </div>`
  })

  document.getElementById("product").innerHTML = display.join(" ");
}

displayMain(data)

let catReduce = data.reduce(function(pre,curr){
  if (pre.includes(curr.category) == false) {
    pre.push(curr.category)
  }
  return pre;
},[]) 
console.log(catReduce);


let catData = catReduce.map(function(item){
  return `<article class="companies" id="com">
  <button class="company-btn" onclick='filter("${item}")'>${item}</button>
</article>`
})
document.getElementById("com").innerHTML = catData.join(" ");
...
function filterPrice(argu) {
 ...
 function readMore(index) {
   let read = document.getElementById(index).innerHTML;
   
   if (read.includes("...readMore") == true) {
    document.getElementById(index).innerHTML = data[index].description.concat("...readLess");
   }
   else{
    document.getElementById(index).innerHTML = data[index].description.slice(0,50).concat("...readMore")
   }
 }
